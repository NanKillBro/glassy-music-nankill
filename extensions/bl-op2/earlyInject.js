...
window.fetch = async function (request, init) {
  ...
  if (
    ...
    try {
      ...
      // Perf: defer parsing to a macrotask (setTimeout(0)) so the browser can
      // render a frame before we JSON.parse a potentially ~1 MB /next response.
      // The original `response` was already returned to YouTube Music, so
      // there is zero urgency to process the intercepted data.
      Promise.all([
        requestBodyPromise,
        ...
        .then(awaitedTexts => {
          // Push heavy JSON.parse off the current microtask queue
          setTimeout(() => {
