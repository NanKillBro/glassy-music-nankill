...
/* =================================================================================================================*/
/* MERGED THEME V14: UPGRADE DYNAMIC BACKGROUND ALGORITHM, HOW IT LOOK AND FEEL                                     */
/* Adds: Update Chaos algorithm, add a bit more details in dynamic background                                       */
/* Fixes: Reduce a bit color banding (maybe?)                                                                       */
...
/* ================================================================================================================ */
/* NOTE: TURN OFF Album Color Theme, Synced Lyric, Ambient Mode, PERFORMANCE IMPROVEMENT (unsure?) TO AVOID BUGS    */
...
/* 1. LỚP ĐỘNG (Chaos Animation)*/
ytmusic-player-page::after {
  content: '';
  position: fixed; /* Cũng dùng fixed để đồng bộ toạ độ */
  top: 0; left: 0; 
  width: 100vw;
  height: 100vh;
  ...
  /* Animation cũ */
  animation: slowRotate 25s linear infinite, scalePulse 8s ease-in-out infinite, appleChaos 60s ease-in-out infinite;
  animation-composition: add;
  
  /* Scale gốc của animation */
  transform: scale(1.3);
  
  /* Nằm trên lớp tĩnh (before) */
  z-index: -1; 
  pointer-events: none;
}

/* Các Keyframes giữ nguyên như cũ */
...
@keyframes appleChaos {
  0% { transform: translate(0, 0); }
  15% { transform: translate(-12%, -7%); }
  27% { transform: translate(-5%, -3%); }
  38% { transform: translate(8%, 12%); }
  45% { transform: translate(3%, 5%); }
  58% { transform: translate(-7%, -10%); }
  65% { transform: translate(3%, 5%); }
  73% { transform: translate(12%, 8%); }
  84% { transform: translate(-9%, 6%); }
  93% { transform: translate(5%, -7%); }
...
function injectStyles() {
    console.log("[NanKill's Skibidi Theme] Injecting MERGED THEME V13...");
